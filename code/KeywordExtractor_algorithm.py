...
class KeywordExtractor:
    ...
    def _add_dependency_edges(self):
        """add_edges"""
        doc = nlp(self.abstract)
        dep_relations = [
            (token.text.lower(), token.head.text.lower(), token.dep_)
            for token in doc
            if token.pos_ in ['NOUN', 'VERB', 'ADJ', 'ADV']
        ]

        for child, head, dep in dep_relations:
            if child not in self.unique_tokens or head not in self.unique_tokens:
                continue

            # assign diff weight
            weight = 1.0
            if dep in ['nsubj', 'dobj', 'amod', 'advmod']:
                weight = 2.0  # more weight for essential grammar

            if self.graph.has_edge(child, head):
                self.graph[child][head]['weight'] += weight
            else:
                self.graph.add_edge(child, head, weight=weight)
    def init_graph(self):
        """
    ...
    def order_nodes(self, method="degree_centrality", to_print=True):
        ...
        # get label
        pos_tags = {
            token: pos for token, pos in
            nltk.pos_tag(self.unique_tokens)
        }
        ...
        # # filter (N* 和 J*)
        filtered = {
            k: v for k, v in sorted_dict.items()
            if pos_tags.get(k, '')[0] in ['N', 'J']
        }


        filtered_sorted_dict = dict(list(filtered.items())[:])
